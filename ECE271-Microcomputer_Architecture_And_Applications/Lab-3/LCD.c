...
int power(int number, int pow)	//Created Power Function to make it easier to do scan check
{
	uint8_t total = 1;
	for(;pow > 0; pow--)
	{
...
void Keypad_Print()	//Function that checks for press of button on keypad and prints that button to LCD
{
	...
	GPIOE->MODER &= ~(GPIO_MODER_MODE10 | GPIO_MODER_MODE11 | GPIO_MODER_MODE12 | GPIO_MODER_MODE13);	//Masked and then set Pin 10, 11, 12, and 13 as Digital Input 
	...
	while(1)
	{
		for(row = 0; row < 4; row++)	//Goes through each row
		{
			...
			GPIOE->ODR |= ((15 - power(2, row)) << 10 ); //Sets the output of the Output to 1111, with a 0 in the position of the number that row represents
			//Checks each row by setting that row to 0, so when you press a button in that row, it will go to 1 and the col to 0

			//Row: 1110 -> 1101 -> 1011 -> 0111
			
			for(col = 0; col < 5; col++)	//Goes through each column
			{
				for(timeDelay = 0; timeDelay < 500; timeDelay++);		//Software Debouncer
				if(col != 3)	//Skips the 3rd column because we are not using Port A Pin 4
				{
					if((GPIOA->IDR & (power(2, col+1))) == 0)	//Checks to see if a button has been pressed in that column
					{
						if(col == 4)	//Point of the step variable is to compensate for the fact that Port A Pin 4 isn't used
						{
							step = 1;
						}
						else
						...
						{
							if(col - step + row*4 == 12)	//If '*' is pressed, then decrement position and print a space
							{
