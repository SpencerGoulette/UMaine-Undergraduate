...
void LCD_DisplayString(uint8_t* ptr){
	int i;
	LCD_Clear();
		for(i=0;i<6;i++) {           // increments 6 times and prints each char
...
void LCD_Display_Name(void){
	...
	LCD->RAM[0] |= 0x80408010;  //prints "MILLET"
	LCD->RAM[1] |= 0x0000000C;
	LCD->RAM[2] |= 0xC080D248;
	LCD->RAM[3] |= 0x0000000C;
	LCD->RAM[4] |= 0x00420040;
	LCD->RAM[5] |= 0x00000000;
	LCD->RAM[6] |= 0x00001208;
	LCD->RAM[7] |= 0x00000000;
	
	LCD->SR |= LCD_SR_UDR;      //sets update display request bit
	
	while(!(LCD->SR & LCD_SR_UDD)); // Wait for Update Display Request Bit
}
...
void LCD_Configure(void){
	...
	while(!(LCD->SR & LCD_SR_RDY));  //waits for the RDY to be checked
}
...
void Scrolling_text(uint8_t* ptr){
	...
	while(1) {    //loops thorugh text
		
		for(i=0;i<6;i++) {   //prints 6 digits at a time then prints the next group
			LCD_WriteChar(ptr+i+position,0,0,i);
			
			if(((GPIOA->IDR & 0x02) == 0x02) | ((GPIOA->IDR & 0x01) == 0x01)) {
				return;
				}
			}
		if(position == 22) { // resets the text to beginning
			position = 0;
		}
		...
		for (i=0;i<125000;i++) {;} //delays
		
		if(((GPIOA->IDR & 0x02) == 0x02) | ((GPIOA->IDR & 0x01) == 0x01)) {  //checks for another request from user
			break;
		}
}
}
