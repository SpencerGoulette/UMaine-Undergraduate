...
{
	// Enable Interrupting when receiving
	...
	// Set USART2 priority to 1
	NVIC_SetPriority(USART2_IRQn, 1);
}

void USART2_IRQHandler(void){
	// Counter and its pointer
	uint32_t counter = 0;
	uint32_t *pCounter = &counter;
	
	// Calls Generic Handler and writes what was typed
	USART_IRQHandler(USART2, buffer1, pCounter);
	USART_Write(USART2, (uint8_t *)buffer1, counter);
}
...
void DMA1_Channel7_IRQHandler(void)
{
	...
	{
		// Reset flag and turn on green LED
		DMA1->IFCR |= DMA_IFCR_CTCIF7;
		Green_LED_On();
	}
}

void DMA_Init(uint8_t *pRxBuffer, uint32_t Size)
{
	// Enable DMA Clock
	...
	// Disable peripheral incremental mode
	DMA1_Channel7->CCR &= ~DMA_CCR_PINC;
	
	// Enable memory incremental mode
	...
	DMA1_Channel7->CMAR = (uint32_t) pRxBuffer;
	
	// Map channel 7 to USART2_TX
